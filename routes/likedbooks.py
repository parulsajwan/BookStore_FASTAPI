...
async def create_customers(customer_id: int, liked: int, db: Session = Depends(get_db)):
    try:
        ...
        if data:
            ...
            fav_books = db.query(FavouriteBooks).filter(
                FavouriteBooks.customer_id == customer_id).first()
            if fav_books:
...
async def get_customer_by_id(id: int, db: Session = Depends(get_db)):
    data = db.query(likedbook.LikedBooks).filter(
        likedbook.LikedBooks.customer_id == id).first()
    if data:
        return data
